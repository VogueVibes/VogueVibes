...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.size-select').forEach(item => {
        item.addEventListener('change', function() {
            var productId = this.dataset.productId;  // Предполагается, что у <select> есть атрибут data-product-id
            var newSize = this.value;
            $.ajax({
                url: 'update_size.php',
                type: 'POST',
                data: {product_id: productId, new_size: newSize},
                dataType: 'json',
